...
@onready var down_door_view: MeshInstance3D = $"../DownStairView/DownDoorView"
...
@onready var up_door_view: MeshInstance3D = $"../UpStairView/UpDoorView"
...
func _ready() -> void:
	...
	var r1_mat: ShaderMaterial = up_stair_view.material_override
	r1_mat.set_shader_parameter("sky_texture", up_stair_viewport.get_texture())
...
func _physics_process(_delta: float) -> void:
	if get_parent().get_parent().get_parent() is SubViewport:
		...
		if parent.own_world_3d:
			return
	if Global.current_world.player0.global_position.y > 3:
		Global.current_world.player0.global_position.y -= 4
	if Global.current_world.player0.global_position.y < -1:
		Global.current_world.player0.global_position.y += 4
